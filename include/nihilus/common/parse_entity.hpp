...
namespace nihilus {
	...
	template<typename value_type, typename element_type> NIHILUS_FORCE_INLINE decltype(auto) get_member(element_type element, value_type& value) {
		using V = std::decay_t<element_type>;
	...
	template<typename value_type, uint64_t current_index = 0> NIHILUS_FORCE_INLINE static constexpr uint64_t find_matching_element(const char* start, size_t length) noexcept {
