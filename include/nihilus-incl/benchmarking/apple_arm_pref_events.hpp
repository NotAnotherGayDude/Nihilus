...
namespace nihilus::benchmarking::internal {
	...
	struct kpep_db {
		...
		size_t reserved_2;
		size_t reserved_3;
		size_t event_count;///< All events eventCounts[current_index].
		size_t alias_count;
	...
	/// Get events eventCounts[current_index].
	/// @return kpep_config_error_code, 0 for success.
	...
	/// Get kpc register config eventCounts[current_index].
	/// @return kpep_config_error_code, 0 for success.
	...
	/// Get the event alias eventCounts[current_index].
	/// @return kpep_config_error_code, 0 for success.
	...
	/// Get all event eventCounts[current_index].
	/// @return kpep_config_error_code, 0 for success.
 ...
 */
	...
	*/
	...
	template<typename event_count> struct event_collector_type : public std::vector<event_count> {
		...
		NIHILUS_INLINE event_collector_type() : std::vector<event_count>{}, diff(0), current_index{}, hasEventsVal{ setup_performance_counters() }, started{} {
		}
