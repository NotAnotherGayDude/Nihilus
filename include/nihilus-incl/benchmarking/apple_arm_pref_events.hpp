...
namespace nihilus::benchmarking {
	...
	template<typename event_count> struct event_collector_type : public aligned_vector<event_count> {
		...
		NIHILUS_INLINE void end(uint64_t bytes_processed) {
			...
			volatile uint64_t cycleEnd = mach_absolute_time();
			...
			if (has_events()) {
				...
				diff					 = end - diff;
				aligned_vector<event_count>::operator[](current_index).cycles_val.emplace(diff.cycles);
				aligned_vector<event_count>::operator[](current_index).instructions_val.emplace(diff.instructions);
				aligned_vector<event_count>::operator[](current_index).branches_val.emplace(diff.branches);
				aligned_vector<event_count>::operator[](current_index).branch_misses_val.emplace(diff.branch_misses);
			} else {
