...
namespace nihilus {
	...
	template<typename output_type, core_types kernel_type> NIHILUS_INLINE static void calculate_gpu_launch_params(output_type& output, dim3& grid_dims, dim3& block_dims) {
		...
		constexpr uint64_t shared_mem_per_block	 = gpu_properties::shared_mem_per_block;
	...
	template<kernel_type_profiles config, model_arches model_arch, model_sizes model_size, model_generations model_generation>
	__global__ void token_embeddings_prompt_eval_time(uint64_t sequence_length, const typename kernel_type_profile_traits<config>::weight_type* __restrict__ weight_data,
		const typename kernel_type_profile_traits<config>::index_type* __restrict__ token_ids,
		typename kernel_type_profile_traits<config>::compute_type* __restrict__ output_data) {
	...
	template<kernel_type_profiles config, model_arches model_arch, model_sizes model_size, model_generations model_generation>
	__global__ void token_embeddings_prompt_eval_time_optimized(uint64_t sequence_length, const typename kernel_type_profile_traits<config>::weight_type* __restrict__ weight_data,
		const typename kernel_type_profile_traits<config>::index_type* __restrict__ token_ids,
		typename kernel_type_profile_traits<config>::compute_type* __restrict__ output_data) {
		static constexpr uint64_t embedding_length	   = model_traits<model_arch, model_size, model_generation>::embedding_length;
		static constexpr uint64_t blocks_per_embedding = embedding_length / 32;
	...
	template<kernel_type_profiles config, model_arches model_arch, model_sizes model_size, model_generations model_generation>
	__global__ void token_embeddings_eval_time(const typename kernel_type_profile_traits<config>::weight_type* __restrict__ weight_data,
		const typename kernel_type_profile_traits<config>::index_type* __restrict__ token_ids,
		typename kernel_type_profile_traits<config>::compute_type* __restrict__ output_data) {
		static constexpr uint64_t embedding_length	   = model_traits<model_arch, model_size, model_generation>::embedding_length;
		static constexpr uint64_t blocks_per_embedding = embedding_length / 32;
	...
	template<kernel_type_profiles config, model_arches model_arch, model_sizes model_size, model_generations model_generation>
	__global__ void token_embeddings_eval_time_optimized(const typename kernel_type_profile_traits<config>::weight_type* __restrict__ weight_data,
		const typename kernel_type_profile_traits<config>::index_type* __restrict__ token_ids,
		typename kernel_type_profile_traits<config>::compute_type* __restrict__ output_data) {
		static constexpr uint64_t embedding_length	   = model_traits<model_arch, model_size, model_generation>::embedding_length;
		static constexpr uint64_t blocks_per_embedding = embedding_length / 32;
	...
	template<kernel_type_profiles config, model_arches model_arch, model_sizes model_size, model_generations model_generation>
	__global__ void token_embeddings_eval_time_single_block(const typename kernel_type_profile_traits<config>::weight_type* __restrict__ weight_data,
		const typename kernel_type_profile_traits<config>::index_type* __restrict__ token_ids,
		typename kernel_type_profile_traits<config>::compute_type* __restrict__ output_data) {
		static constexpr uint64_t embedding_length	   = model_traits<model_arch, model_size, model_generation>::embedding_length;
		static constexpr uint64_t blocks_per_embedding = embedding_length / 32;
	...
	struct kernel_dispatcher_impl<config, core_traits_type, device_types::gpu, 4, core_types::token_embeddings, processing_phases::eval_time> {
		NIHILUS_INLINE static void impl(core_traits_type& params, int64_t thread_index, int64_t thread_count) {
			...
			if constexpr (embedding_length <= 512) {
				...
				token_embeddings_eval_time_single_block<kernel_type_profile, model_arch, model_size, model_generation>
					<<<1, threads, shared_mem_size>>>(weight_data, token_ids, output_data);
				...
				token_embeddings_eval_time<kernel_type_profile, model_arch, model_size, model_generation><<<blocks, threads>>>(weight_data, token_ids, output_data);
			} else {
				const uint64_t threads = 256;
				const uint64_t blocks  = min(static_cast<uint64_t>(8), (blocks_per_embedding + 3) / 4);

				token_embeddings_eval_time_optimized<kernel_type_profile, model_arch, model_size, model_generation><<<blocks, threads>>>(weight_data, token_ids, output_data);
			}
