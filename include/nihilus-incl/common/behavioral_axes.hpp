...
namespace nihilus {
	...
	template<model_config config, typename base_type_new> struct total_bytes_collector {
		...
		NIHILUS_INLINE static constexpr void impl(const base_type& base, memory_plan<config>& mem_plan) {
	...
	template<model_config config, typename base_type_new, processing_phase phase> struct per_block_thread_function {
		...
		using return_type																			   = bool;
		NIHILUS_INLINE static constexpr bool filter() {
			return base_type::core_type != core_types::weights && base_type::core_type != core_types::global_inputs && base_type::core_type != core_types::global_outputs &&
	...
	template<model_config config, typename base_type_new, processing_phase phase> struct global_output_thread_function {
		...
		using return_type																					   = bool;
		NIHILUS_INLINE static constexpr bool filter() {
			return base_type::core_type == core_types::global_outputs;
		}
		...
		NIHILUS_INLINE static void impl(base_type& parse_core, int64_t thread_index_new, int64_t thread_count) {
			...
			if constexpr (config.dev) {
				std::stringstream stream{};
				stream << "[DEBUG] Thread " << thread_index_new << " about to wait on barrier with " << thread_count << " expected threads" << std::endl;
				log<log_levels::status>(stream.str());
			}
			parse_core.latch.arrive_and_wait();
	...
	template<model_config config, typename base_type_new, processing_phase phase> struct main_thread_function {
		NIHILUS_INLINE main_thread_function() noexcept														   = default;
		NIHILUS_INLINE main_thread_function& operator=(const main_thread_function&) noexcept				   = delete;
		NIHILUS_INLINE main_thread_function(const main_thread_function&) noexcept							   = delete;
		NIHILUS_INLINE main_thread_function& operator=(main_thread_function&&) noexcept						   = delete;
		NIHILUS_INLINE main_thread_function(main_thread_function&&) noexcept								   = delete;
		using base_type																						   = base_type_new;
		using return_type																					   = bool;
		NIHILUS_INLINE static constexpr bool filter() {
