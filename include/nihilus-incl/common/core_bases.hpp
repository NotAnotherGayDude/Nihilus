...
namespace nihilus {
	...
	template<model_config config_new, typename enum_type> static constexpr get_core_bases_t<config_new, enum_type> core_bases_val{};
	...
	template<model_config config, typename enum_type> struct core_bases_traits {
		static constexpr memory_plan total_required_bytes{ []() {
			...
			core_bases_val<config, enum_type>.template impl<total_bytes_collector>(return_value);
			return return_value;
		}() };
	};
}
