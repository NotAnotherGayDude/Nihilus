...
namespace nihilus {
	...
	template<typename config_type, typename... value_type> struct get_nihilus_cathedral {
		using type = nihilus_cathedral<config_type, value_type...>;
	};

	template<typename config_type, size_t... index> struct get_nihilus_cathedral<config_type, std::index_sequence<index...>> {
		using type = nihilus_cathedral<config_type, core_traits_old<config_type, static_cast<core_types>(index)>...>;
	};

	template<typename config_type, typename... value_type> using get_core_base_t = typename get_nihilus_cathedral<config_type, value_type...>::type;

	template<typename config_type> using get_nihilus_cathedral_t = typename get_nihilus_cathedral<config_type, std::make_index_sequence<static_cast<uint64_t>(core_types::count)>>::type;
